...
let process_lines parse_code_and_perform_diff version dirname filename lines =
  let block_end start_line before minus plus after =
    ...
    then
      begin
        ...
        parse_code_and_perform_diff (version, dirname, filename) start_line 
          (current_region())
          m p
...
let hunks_of_patch unparsed_patch = 
